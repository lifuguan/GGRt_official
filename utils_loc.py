...
def colorize_np_1(x, cmap_name='jet', mask=None, range=None, append_cbar=False, cbar_in_image=False, cbar_precision=2):
    '''
...
def colorize_np(x, cmap_name='jet', mask=None, range=None, append_cbar=False, cbar_in_image=False, cbar_precision=2):
    ...
    if range is not None:
        ...
        vmin, vmax = np.percentile(x[x>0], (1, 100))
        vmin=np.log(vmin)
        vmax=np.log(vmax)
        vmax += TINY_NUMBER
    x = np.log(x)
...
def colorize_1(x, cmap_name='jet', mask=None, range=None, append_cbar=False, cbar_in_image=False):
    device = x.device
    x = x.cpu().numpy()
    if mask is not None:
        mask = mask.cpu().numpy() > 0.99
    ...
    x = colorize_np_1(x, cmap_name, mask, range, append_cbar, cbar_in_image)
    x = torch.from_numpy(x).to(device)
    return x
